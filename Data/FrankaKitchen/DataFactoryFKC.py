...
class Iter_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for g in ['achieved_goal', 'desired_goal']:
            for t in ['kettle', 'light switch', 'microwave', 'slide cabinet']:
...
def FrankaKitchen():
    ...
    trainset, valset, testset = random_split(dataset, [15, 4, 0])
    ...
    dl_test = dl_val    #reuses given dataset shortage
...
def score(model, test_runs=10):
    ...
    with torch.no_grad():
        ...
        dset = minari.load_dataset('kitchen-complete-v1') 
        environments = []
        observations = []
