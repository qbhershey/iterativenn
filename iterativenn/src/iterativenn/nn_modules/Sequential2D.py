...
class Sequential2D_Identity(torch.nn.Identity):
    def __init__(self, *args, in_features=None, out_features=None, **kwargs):
        super(Sequential2D_Identity, self).__init__(*args, **kwargs)
...
class Sequential2D(torch.nn.Module):
    ...
    *Note, it requires that the modules have a "forward" method and that they have a "in_features" and "out_features" attribute.*
    ...
    1) MaskedLinear fits in nicely.
    2) Linear layers can be used as well.
    3) As can another Sequential2D!  This allows the blocks to be nested and quite arbitrary.

    In fact, if every entry in the list is a MaskedLinear, then this is just equivalent to a MaskedLinear, which is nice for regression testing.
        ...
        for i, row in enumerate(block_types):
            ...
            for j, block_type in enumerate(row):
                if block_type is None:
                    ...
                    # This is a module that has already been created.  It is the responsibility of the user to make sure that the module is compatible with the Sequential2D
                    # The trainability of the module is set by the user bty default.  If the user wants to freeze the module, they can set the 'trainable' key to False.
                    module = cfg['block_kwargs'][i][j]['module']
                    if ('trainable') in cfg['block_kwargs'][i][j].keys() and (not cfg['block_kwargs'][i][j]['trainable']):
                        for param in module.parameters():
                            param.requires_grad = False
                    blocks_row.append(module)
                elif block_type == 'WandBModule':
                    # This is cool!  This loads in a module that was saved by wandb.
                    save_dict = json.load(open(cfg['block_kwargs'][i][j]['run_path_file'], 'r'))
                    model_restore = wandb.restore(save_dict['save_model_name'], 
                                                  run_path=save_dict['path'])
                    model = torch.load(model_restore.name)
                    blocks_row.append(model)
                else:
