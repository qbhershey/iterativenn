...
from iterativenn.utils.gpt_config_utils import (GPT2BlockUpdater)
from iterativenn.utils.logger_factory import LoggerFactory
from iterativenn.lit_callbacks.cache import CustomWriter

import torch
import os

def predict(cfg_logger):
    ...
    GPT_LAYERS = 8  # plus 1 for language model head # 3
    ...
    gpt2_block_updater = GPT2BlockUpdater(size=GPT_LAYERS)  # Initialize GPT2BlockUpdater with size of GPT_LAYERS
    block_types = gpt2_block_updater.get_block_types()
    block_kwargs = gpt2_block_updater.get_block_kwargs()
    gpt2_block_updater.init_gpt2_blocks(mode='block_predict')  # Initialize GPT2BlockUpdater with GPT2 blocks in subdiagonal format
    ...
    cfg = {
        ...
        "callbacks": {
            ...
            "output": {
                "func": "all",
                "idx_list": range(gpt_dim * (GPT_LAYERS - 1), (gpt_dim * GPT_LAYERS) )
            }
        }
    }
    model = Sequential2D.from_config(cfg["sequential2D"])
    callbacks = ConfigCallbacks(cfg["callbacks"])
    model_nlp = IteratedModelGPT2(model, callbacks, gpt_embed, normalize_loss=False, optimizer='Adam_customLR',
                                  iterations=GPT_LAYERS)
    ...
    trainer = Trainer(
        ...
        callbacks=[EarlyStopping(monitor="validation_loss", mode="min", patience=2), pred_writer]
    )

    # trainer.predict(model_nlp, data_module) # to get the predictions for only predict_dataloader

    # to get the predictions for all the dataloaders
    trainer.predict(model_nlp, dataloaders=[data_module.train_dataloader(),
