...
def runner_main(cfg_logger):
    ...
    print(
        f"GPT_LAYERS_WINDOW: {GPT_LAYERS_WINDOW}, GPT_LAYERS_START: {GPT_LAYERS_START}, GPT_LAYERS_END: {GPT_LAYERS_END}")
    ...
    operator_module = sparse_sequential2D_from_masked_linear(
        dim_z=gpt_dim)  # Initialize Sequential2D with dim_z=gpt_dim
    ...
    model_nlp = IteratedModelGPT2(model, callbacks, gpt_embed, normalize_loss=False,
                                  optimizer='Adam_customLR', iterations=GPT_LAYERS_WINDOW)
