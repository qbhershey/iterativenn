...
def runner_main(cfg_logger):
    ...
    GPT_LAYERS = gpt_config.n_layer+1+1 # plus 1 for language model head
    ...
    operator_module = HadamardIdentity(in_features=gpt_dim, bias=True)
    # grow by adding identity blocks
    operator_ids = [(7, 8)] # (1, 2)
    gpt2_block_updater.init_gpt2_blocks_insert_hidentity(operator_module, operator_ids[0])
    gpt2_block_updater.update_block_lrs(1, 2, 1e-2)

    # gpt2_block_updater.init_gpt2_blocks()
    # for i in range(len(operator_ids)):
    #     operator_idx, operator_idy = operator_ids[i]
    #
    #     # copy the current block to the next block
    #     current_module_type = gpt2_block_updater.get_block_type(operator_idx, operator_idy)
    #     current_module_kwargs = gpt2_block_updater.get_block_kwarg(operator_idx, operator_idy)
    #
    #     # insert the operator block
    #     gpt2_block_updater.update_block_types(operator_idx, operator_idy, 'Module')
    #     gpt2_block_updater.update_block_kwargs(operator_idx, operator_idy, {'module': operator_module})
    #     gpt2_block_updater.update_block_lrs(operator_idx, operator_idy, operator_lrs[i])
    #
    #     # insert the current block to the next block
    #     gpt2_block_updater.update_block_types(operator_idx, operator_idy, current_module_type)
    #     gpt2_block_updater.update_block_kwargs(operator_idx+1, operator_idy+1, current_module_kwargs)
    #     gpt2_block_updater.update_block_lrs(operator_idx, operator_idy, operator_lrs[i])
    ...
    model_nlp = IteratedModelGPT2(model, callbacks, gpt_embed, normalize_loss=False, optimizer='Adam_customLR', iterations=GPT_LAYERS)
    ...
    trainer = Trainer(
        accelerator='auto',
        devices =2 if torch.cuda.is_available() else None,  # limiting got iPython runs
        max_epochs=20,
        logger=logger,
